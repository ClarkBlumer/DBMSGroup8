...
class Reports_model extends CI_Model{
    ...
    function getHighestBudgetForPending(){
        ...
        $this->db->from("TBL_PROPOSAL");
        $this->db->join("TBL_PROP_PRI_COURSES", "(PROPMAX(PROP_BUDGE_REQUESTED)ID)");
        //$this->db->using("(PROPMAX(P1.PROP_BUDGET_REQUESTED)ID)");
        $this->db->group_by("INSTITUTION");
        $this->db->where("PROP_OFFER_STATUS='pend'");
        $query = $this->db->get_where();
        //$query = $this->db->query("SELECT MAX(P1.PROP_BUDGET_REQUESTED) FROM TBL_PROPOSAL INNER JOIN TBL_PROP_PRI_COURSES USING(PROPMAX(P1.PROP_BUDGET_REQUESTED)ID) WHERE PROP_OFFER_STATUS='pend'");
        return $query->result_array();
    }
}
